...
public class Util {
	...
	public static List<Long> getMostRecurrentElements(final List<Long> elements, final List<Long> restrictions) {
		...
		for (Long element: elements) {
			if (restrictions == null || restrictions.contains(element)) {
				...
				if (count == null) {
					iterations.put(element, Integer.valueOf(1));
				} else {
					iterations.put(element, Integer.valueOf(count + 1));
				}
			}
		}
	 ...
	 * @return
	 */
	public static String getUsersNames(final List<User> users, final List<Long> usersIds) {
		...
		for (int i = 0; i < usersIds.size(); i++) {
			...
			User user = getObjectById(users, usersIds.get(i));
			if (user != null) {
